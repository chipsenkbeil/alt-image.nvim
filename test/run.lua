...
local M = { suites = {}, current = nil, before = nil, after = nil, passed = 0, failed = 0 }
...
_G.assert = setmetatable({
    ...
    matches = function(pat, s, msg)
        if type(s) ~= "string" or not s:find(pat) then
            error((msg or "") .. " pattern " .. tostring(pat) .. " not in " .. vim.inspect(s), 2)
        end
    end,
    is_true = function(v, msg)
        if v ~= true then
            error((msg or "") .. " want true,  got " .. vim.inspect(v), 2)
        end
    end,
    is_false = function(v, msg)
        if v ~= false then
            error((msg or "") .. " want false, got " .. vim.inspect(v), 2)
        end
    end,
    is_nil = function(v, msg)
        if v ~= nil then
            error((msg or "") .. " want nil,   got " .. vim.inspect(v), 2)
        end
    end,
}, {
    __call = function(_, v, msg)
        return _orig_assert(v, msg)
    end,
...
for _, suite in ipairs(M.suites) do
    ...
    for _, t in ipairs(suite.tests) do
        ...
        if t.before then
            before_ok, before_err = pcall(t.before)
        end
