...
describe("alt-img.sixel set/get/del", function()
    local img, png_bytes
...
describe("alt-img.sixel relative=ui clipping", function()
    local img, png_bytes
    before_each(function()
        H.setup_capture()
        img = H.fresh_provider("sixel")
...
describe("alt-img.sixel _supported", function()
    before_each(function()
    ...
    it("returns true when TERM matches *sixel*", function()
        H.with_env({ TERM = "xterm-sixel" }, function()
            assert.is_true(require("alt-img.sixel")._supported())
        end)
    end)
    ...
    it("returns false for Apple Terminal explicitly", function()
        H.with_env({ TERM_PROGRAM = "Apple_Terminal", TERM = "xterm-256color" }, function()
            assert.is_false(require("alt-img.sixel")._supported({ timeout = 10 }))
        end)
    end)
    ...
    it("returns true via WT_SESSION (Windows Terminal)", function()
        H.with_env({ WT_SESSION = "fake-uuid", TERM = "xterm-256color", TERM_PROGRAM = false }, function()
...
describe("alt-img.sixel relative=editor", function()
    local img, png_bytes
    before_each(function()
        H.setup_capture()
        img = H.fresh_provider("sixel")
    ...
    it("opens a floating-window carrier and emits at its screen pos", function()
        ...
        local id = img.set(png_bytes, { relative = "editor", row = 5, col = 10, width = 4, height = 4 })
...
describe("alt-img.sixel relative=buffer", function()
    local img, png_bytes
    before_each(function()
        H.setup_capture()
        img = H.fresh_provider("sixel")
    ...
    it("does not error when buffer shrinks below the extmark", function()
        ...
        local id = img.set(png_bytes, { relative = "buffer", buf = buf, row = 4, col = 1, width = 4, height = 4 })
    ...
    it("hides the image when its anchor line is deleted", function()
        ...
        local id = img.set(png_bytes, { relative = "buffer", buf = buf, row = 2, col = 1, width = 4, height = 4 })
    ...
    it('defaults relative to "buffer" when opts.buf is set', function()
        ...
        local id = img.set(png_bytes, { buf = buf, row = 1, col = 1, width = 4, height = 4 })
    ...
    it("resolves buf=0 to the current buffer", function()
        ...
        local id = img.set(png_bytes, { buf = 0, row = 1, col = 1, width = 4, height = 4 })
    ...
    it("emits the image once per window showing the buffer", function()
        ...
        vim.cmd("split")
        H.reset_capture()
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 1, col = 1, width = 4, height = 4 })
        local cap = H.captured()
    ...
    it("returns cropped src when image extends past window bottom", function()
        ...
        local sixel_mod = require("alt-img.sixel")
    ...
    it("returns cropped src.w when image extends past window right edge", function()
        ...
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 1, col = 1, width = 10, height = 4 })
        local carrier = require("alt-img._core.carrier")
        local sixel_mod = require("alt-img.sixel")
        local positions = carrier.get_positions(sixel_mod, id)
        assert.is_true(#positions >= 1)
    ...
    it("returns cropped src.h when image extends past window top", function()
        ...
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 3, col = 1, width = 4, height = 8 })
    ...
    it("returns cropped src.x when image extends past window left edge", function()
        ...
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 1, col = 2, width = 10, height = 4 })
        local carrier = require("alt-img._core.carrier")
        local sixel_mod = require("alt-img.sixel")
        local positions = carrier.get_positions(sixel_mod, id)
    ...
    it("returns empty position list when anchor scrolls above window", function()
        ...
        local positions = require("alt-img._core.carrier").get_positions(require("alt-img.sixel"), id)
