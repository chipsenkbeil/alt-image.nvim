...
describe("alt-img.iterm2 set/get/del", function()
    ...
    it("preserveAspectRatio=1 when only one dim given", function()
        img.set(read_fixture(), { row = 1, col = 1, width = 4 })
        local r = H.parse_iterm2_seq(H.captured():match("\027%]1337;File=[^\007]*\007"))
        assert.equals("1", r.args.preserveAspectRatio)
    end)

    it("get returns opts; nil for missing id", function()
        local id = img.set(read_fixture(), { row = 2, col = 3, width = 4, height = 4 })
    ...
    it('defaults relative to "buffer" when opts.buf is set', function()
        ...
        local id = img.set(read_fixture(), { buf = buf, row = 1, col = 1, width = 4, height = 4 })
    ...
    it("resolves buf=0 to the current buffer", function()
        ...
        local id = img.set(read_fixture(), { buf = 0, row = 1, col = 1, width = 4, height = 4 })
...
describe("alt-img.iterm2 del(math.huge)", function()
    ...
    it("clears all placements", function()
        local a = img.set(read_fixture(), {})
        local b = img.set(read_fixture(), {})
...
describe("alt-img.iterm2 relative=editor", function()
    ...
    it("opens a floating-window carrier and emits at its screen pos", function()
        ...
        local id = img.set(read_fixture(), { relative = "editor", row = 5, col = 10, width = 4, height = 4 })
...
describe("alt-img.iterm2 relative=buffer", function()
    ...
    it("does not error when buffer shrinks below the extmark", function()
        ...
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 4, col = 1, width = 4, height = 4 })
    ...
    it("hides the image when its anchor line is deleted", function()
        ...
        local id = img.set(read_fixture(), { relative = "buffer", buf = buf, row = 2, col = 1, width = 4, height = 4 })
